...
5. 성능
   ...
   - 모바일 재방문을 위해 manifest, 아이콘, 서비스워커, 오프라인 안내 페이지를 유지합니다.
...
1. Search Console URL 접두어 속성 등록과 sitemap 제출
2. Search Console 검색어를 기준으로 `CONTENT_PLAN.md` 후보 보강
3. 바둑/오목 문제 즐겨찾기와 이어풀기
4. 모바일 대국 화면에서 광고가 들어가도 오클릭이 나지 않는 안전 영역 검증
5. 로컬 학습 기록 백업/복원
6. Search Console 소유권 확인 뒤 AdSense 신청 패킷 실행
...
- `SUBMISSION_PACKET.md`: Search Console과 AdSense 신청 때 복사할 URL과 명령 모음
...
- `scripts/check-pwa.cjs`: manifest, 앱 아이콘, 서비스워커, 오프라인 페이지 연결 점검
...
- `scripts/sync-feed.cjs`: 최신 50개 학습 글 RSS feed 동기화 검사와 자동 갱신
...
- `scripts/create-operations-review.cjs`: 월간 운영 리뷰 파일에 수익화/콘텐츠/계정 작업 스냅샷 기록
